...
flags.DEFINE_string(
    "export_dir", None,
    "The output directory where the model checkpoints will be exported.")
...
flags.DEFINE_bool("do_export", False, "Whether to save model.")
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  while len(a_input_ids) < max_seq_length:
    a_input_ids.append(0)
  ...
  # Zero-pad up to the sequence length.
  while len(b_input_ids) < max_seq_length:
    b_input_ids.append(0)
    b_input_mask.append(0)
    b_segment_ids.append(0)

  assert len(b_input_ids) == max_seq_length
  assert len(b_input_mask) == max_seq_length
  assert len(b_segment_ids) == max_seq_length

  label = int(example.label)/(max_score - min_score)
  if ex_index < 5:
    tf.logging.info("*** Example ***")
    tf.logging.info("guid: %s" % (example.guid))
    tf.logging.info("a_tokens: %s" % " ".join(
        [tokenization.printable_text(x) for x in a_tokens]))
    tf.logging.info("a_input_ids: %s" % " ".join([str(x) for x in a_input_ids]))
    tf.logging.info("a_input_mask: %s" % " ".join([str(x) for x in a_input_mask]))
    tf.logging.info("a_segment_ids: %s" % " ".join([str(x) for x in a_segment_ids]))
    tf.logging.info("b_tokens: %s" % " ".join(
        [tokenization.printable_text(x) for x in b_tokens]))
    tf.logging.info("b_input_ids: %s" % " ".join([str(x) for x in b_input_ids]))
    tf.logging.info("b_input_mask: %s" % " ".join([str(x) for x in b_input_mask]))
    tf.logging.info("b_segment_ids: %s" % " ".join([str(x) for x in b_segment_ids]))
    tf.logging.info("label: %s (id = %d)" % (example.label, label))
...
def file_based_convert_examples_to_features(
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    features["a_input_ids"] = create_int_feature(feature.a_input_ids)
    features["a_input_mask"] = create_int_feature(feature.a_input_mask)
    features["a_segment_ids"] = create_int_feature(feature.a_segment_ids)
    features["b_input_ids"] = create_int_feature(feature.b_input_ids)
    features["b_input_mask"] = create_int_feature(feature.b_input_mask)
    features["b_segment_ids"] = create_int_feature(feature.b_segment_ids)
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
  ...
  name_to_features = {
      "a_input_ids": tf.FixedLenFeature([seq_length], tf.int64),
      "a_input_mask": tf.FixedLenFeature([seq_length], tf.int64),
      "a_segment_ids": tf.FixedLenFeature([seq_length], tf.int64),
      "b_input_ids": tf.FixedLenFeature([seq_length], tf.int64),
      "b_input_mask": tf.FixedLenFeature([seq_length], tf.int64),
      "b_segment_ids": tf.FixedLenFeature([seq_length], tf.int64),
...
def main(_):
  ...
  if FLAGS.do_export:
      def serving_input_receiver_fn():
          ...
          b_input_ids = tf.placeholder(dtype=tf.int64, shape=[None, FLAGS.max_seq_length], name='b_input_ids')
          b_input_mask = tf.placeholder(dtype=tf.int64, shape=[None, FLAGS.max_seq_length], name='b_input_mask')
          b_segment_ids = tf.placeholder(dtype=tf.int64, shape=[None, FLAGS.max_seq_length], name='b_segment_ids')
          label_ids = tf.placeholder(dtype=tf.float32, shape=[None, ], name='unique_ids')
          receive_tensors = {'a_input_ids': a_input_ids, 'a_input_mask': a_input_mask, 'a_segment_ids': a_segment_ids,
                  'b_input_ids': a_input_ids, 'b_input_mask': b_input_mask, 'b_segment_ids': b_segment_ids,
                  'label_ids': label_ids}
          features = {'a_input_ids': a_input_ids, 'a_input_mask': a_input_mask, 'a_segment_ids': a_segment_ids,
                  'b_input_ids': b_input_ids, 'b_input_mask': b_input_mask, 'b_segment_ids': b_segment_ids, "label_ids": label_ids}
  ...
  if FLAGS.do_predict:
    ...
    with tf.gfile.GFile(output_submit_file, "w") as sub_writer:
      num_written_lines = 0
      tf.logging.info("***** Predict results *****")
      for (i, (example, prediction)) in\
          enumerate(zip(predict_examples, result)):
        actual_label = prediction["predictions"]
